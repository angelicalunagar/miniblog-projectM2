...
Para visualizarla, puedes:  Copiar el contenido de openapi.yaml en el Swagger Editor.O utilizar una extensión de VS Code como "Swagger Viewer".

## Endpoints de la API

La API expone las siguientes rutas RESTful para la gestión de autores y publicaciones[cite: 1]:

### Autores (`/authors`)
* **GET `/authors`**: Obtiene la lista de todos los autores registrados[cite: 1].
* **POST `/authors`**: Crea un nuevo autor en el sistema[cite: 1].
* **GET `/authors/{id}`**: Obtiene los detalles de un autor específico mediante su ID[cite: 1].
* **PUT `/authors/{id}`**: Actualiza la información de un autor existente[cite: 1].
* **DELETE `/authors/{id}`**: Elimina un autor del sistema[cite: 1].

### Publicaciones (`/posts`)
* **GET `/posts`**: Obtiene la lista completa de todas las publicaciones[cite: 1].
* **POST `/posts`**: Crea una nueva publicación[cite: 1].
* **GET `/posts/{id}`**: Consulta el detalle de una publicación específica por su ID[cite: 1].
* **PUT `/posts/{id}`**: Actualiza el contenido o los datos de una publicación existente[cite: 1].
* **DELETE `/posts/{id}`**: Elimina una publicación del miniblog[cite: 1].
